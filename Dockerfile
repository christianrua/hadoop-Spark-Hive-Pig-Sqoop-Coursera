...
RUN cp postgresql-42.6.0.jar $SQOOP_HOME/lib/
...
RUN tar xvf spark-3.2.4-bin-hadoop2.7.tgz
#RUN mv spark-3.0.3-bin-hadoop2.7 /usr/local/spark
RUN mv spark-3.2.4-bin-hadoop2.7 /usr/local/spark
...
RUN echo "export JAVA_HOME=/usr/bin/java" >> $SPARK_HOME/conf/spark-env.sh
...
# USER root
# RUN apt-get install -y mysql-server systemd-sysv
